...
Loại này khác xa hoàn toàn với loại Clustered Index  chính là cái cách mà lá trên B-tree hoạt động.
Nút lá trên Clustered Index thì nó sẽ có dữ liệu còn trên Non-Clustered Index thì không nó chỉ có cái ID định danh dựa trên các tình huống riêng biệt.

Nhìn vào ảnh này sẽ rõ : 

![image](https://user-images.githubusercontent.com/63473793/123789837-c9e29400-d907-11eb-9202-fb4ce4d8bdc0.png)

Nhìn ra thì chúng ta có 2 tình huống ngớ ngẩn ở đây :

1. nếu nó ko có Clustered Index thì sao : thì Mã định danh hàng (RID) của Non-Clustered Index sẽ trỏ đến bảng heap. VÀ nó không là gì ngoài một bảng không có chỉ mục. Trong bảng heap, nó sẽ tìm kiếm bản ghi từng hàng cho đến khi tìm thấy dữ liệu.

2. Nếu nó có Clustered Index thì sao : hì Mã định danh hàng (RID) của Non-Clustered Index sẽ trỏ đến khóa Clustered Index và khóa được lập chỉ mục đó được sử dụng để tìm kiếm dữ liệu.

### Vậy nếu nó không lưu trữ dữ liệu mà nó chỉ lưu RID vậy thì chắc sẽ co nhiều Non-Clustered Index đúng không?

Chính xác là như vậy.Vì các Non-Clustered Index được tạo riêng biệt với dữ liệu thực tế, do đó, một bảng có thể có nhiều hơn một chỉ mục không phân nhóm trong SQL Server.
Trong INdex table, dữ liệu được lưu trữ theo thứ tự tăng dần hoặc giảm dần của khóa chỉ mục, không tạo ra bất kỳ ảnh hưởng hoặc thay đổi nào đối với dữ liệu thực được lưu trữ trong bảng. Trong SQL Server, chúng ta có thể tạo tối đa 999 chỉ mục không phân cụm cho một bảng.

### Chiến thôi anh zai. Chiến luôn cho (lóng)

Bắt đầu quất: 

- ở đây chúng ta sẽ sử dụng bảng [Tinh] trong [XaHuyenTinh]
- lúc này chỉ có Clustered
 sử dụng câu lệnh sau để kiểm tra : EXECUTE sp_helpindex [Tinh]

![image](https://user-images.githubusercontent.com/63473793/123791645-e8498f00-d909-11eb-8e13-4647125bfbc8.png)

Chúng ta đã có chỉ mục do primary key tạo ra nó chính là một Clustered và chúng ta có thể đổi Clustered nảy theo công thứ chứ danh sau 

```sql
CREATE CLUSTERED INDEX <index_name>
ON <table_name>(<column_name> ASC/DESC)
```
Tất nhiên bạn không chạy dk câu lệnh trên rồi vì chỉ có tôi chạy dk thoy :D

Vào phần chính : Non-clustered indexes
...
ON <table_name>(<column_name> ASC/DESC)
```
Chạy lệnh :  SELECT * FROM [Tinh]

![image](https://user-images.githubusercontent.com/63473793/123792332-b38a0780-d90a-11eb-8467-d6d4d2b8eac2.png)

Quất INDEX Chạy Lệnh  CREATE NONCLUSTERED INDEX IX_tblTinh_Name ON [Tinh](NAMETP ASC)

Oh fuck sao trông nó lại như thế này.

Chạy lệnh :  SELECT * FROM [Tinh]

![image](https://user-images.githubusercontent.com/63473793/123792535-edf3a480-d90a-11eb-9237-6d9faf2196bd.png)

Hiểu đơn giản cách mà SQL tìm kiếm như sau : xác định Non-clustered indexes  => Nhận biết clustered indexes => Lấy dữ liệu. Chấm hết!
## 3.Unique Index
